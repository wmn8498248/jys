<template>
	<view class="text-white">
		...
		<view class="m-30">
			<navigator open-type="reLaunch"  class="py-20 border-bottom" v-for="(item, index) in quotation" :key="index" :url="`/pages/transaction/index?from=market&currency_name=${item.currency_name}&legal_name=${item.legal_name}&currency_id=${item.currency_id}`">
