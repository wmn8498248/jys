<template>
	<view>
		...
		<view class="mx-30 mb-30 py-30 border-radius-20 box-shadow bg-white animated text-deepblue" v-if="showFliter"  :class="showFliterAnimate ? 'fadeInDown' : 'fadeOutUp'" >
			...
			<view class="u-border-bottom p-30" v-for="(item, index) in filters" :key="index" style="white-space: nowrap;">
				<text class="d-block font-size-32 font-weight-bold">{{item.name}}</text>
				<scroll-view :scroll-x="true" class="mt-30">
					<view class="filter-item d-inline-block" :class="{'active':currentNav[item.value] == el.value}" @click="currentNav[item.value] = el.value" v-for="(el, index) in item.child" :key="index">{{el.name}}</view>
				</scroll-view>
			</view>
		...
		<view class="mx-50 mb-30 d-grid-columns-2" style="grid-column-gap:20rpx">
			<view class="w-100 overflow-hidden" v-for="(item,index) in list" :key="index">
