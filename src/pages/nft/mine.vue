	...
	<view>
		...
		<view class="mx-30">
			...
			<view class="mt-40 d-grid-columns-2 " style="grid-column-gap:20rpx" v-if="activeNav == 0">
				<view class="mb-40 animated fadeInLeft" v-for="(item, index) in position" :key="index" @click="$utils.jump('/pages/nft/myNft?id='+item.id)">
			...
			<view class="mt-40" style="grid-column-gap:20rpx" v-if="activeNav == 1">
				<view class="mb-24 border-radius-10 d-flex-between-center p-30" v-for="(item, index) in order" :key="index" style="background-image: linear-gradient(225deg, #edf1f9 0%, #e4efe9 100%);">
			...
			<view class="mt-40" style="grid-column-gap:20rpx" v-if="activeNav == 2">
				<view class="mb-24 box-shadow border-radius-10 d-flex-between-center p-30"  v-for="(item,index) in needPayList" :key="index" @click="read(index)" :class=" item.is_read == 1 ? 'bg-success-filter' : 'bg-error-filter' " >
