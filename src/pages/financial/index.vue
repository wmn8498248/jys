			...
			<view style="height: 1500rpx;overflow-y: auto;">
				...
				<view class="mx-30 mt-26 z-index-10 mb-70">
					<view class="d-flex align-items-center justify-content-between ">
						<view class="bg-white d-flex px-40 py-30 border-radius-10 align-items-center w-49" v-for="(item ,index) in menu.slice(0,2)" :key="index" @click="homeNavJump(item.url,item.open_type)">
					...
					<view class="d-flex align-items-center justify-content-center bg-white border-radius-10 py-20 mt-10" v-for="item ,index) in menu.slice(2,3)" :key="index" @click="homeNavJump(item.url,item.open_type)">
						<image :src="`../../static/image/icon/${item.icon}.png`" class="d-block" style="width: 50rpx;height: 50rpx;" mode=""></image>
						<text class="d-block font-size-32 text-black ml-20 font-weight-bold">{{item.name}}</text>
					</view>
				</view> -->
				...
				<view class="mb-50" v-if="invest.length">
					...
					<view class="mx-30 my-20 bg-white p-30 border-radius-20 bg-white" v-for="(item, index) in invest[activeInvest].slice(0,3)" :key="index" @click="$utils.jump('/pages/invest/purchase?id='+item.id)">
				...
				<view class="mb-50" v-if="lockming.length">
					...
					<view class="m-30 mt-20 high-quality-project">
						<swiper style="height: 390rpx;" :acceleration="true" :display-multiple-items="2" :indicator-dots="false"
							...
							<swiper-item v-for="(item,index) in lockming.slice(0,5)" :key="index">
				...
				<view class="mb-50" v-if="ieo.length">
					...
					<view class="mx-30 my-20 p-30 box-shadow border-radius-20 mb-20 bg-white text-white" v-for="(item, index) in ieo.slice(0,3)" :key="index">
