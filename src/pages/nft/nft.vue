	...
	<view>
		...
		<view class="fixed-nav text-center animated fadeInUp" @click="clickFixedNav" >
			<view class="px-30 d-flex align-items-center h-100">
				<view v-for="(item,index) in nav" :key="index" @click.stop="selectNav(index)" class="w-25 text-center d-block" :class="'navitem-' + index" :style="{color:activeNav == index  ? '#fff' : '#333'}">
