				...
				<view class="m-30 market text-white">
					<view class="mt-22" v-show="currentNav == 0">
						<view class="title d-grid" style="grid-template-columns:1.1fr 1fr 1fr;">
							<view class="d-flex align-items-center text-warning font-weight-bold" v-for="(item,index) in quotationNav" :key="index" :class="item.align">
								<text>{{item.name}}</text>
							</view>
						</view>
						<navigator class="market-item" v-for="(item, index) in optionalList" :key="index" open-type="reLaunch" :url="`/pages/transaction/index?from=market&currency_name=${item.currency_name}&legal_name=${item.legal_name}&currency_id=${item.currency_id}`">
							<view class="left">
								<text class="d-block">
					...
					<view class="mt-22" v-show="currentNav == 1">
						<view class="title d-grid" style="grid-template-columns:1.1fr 1fr 1fr;">
							<view class="d-flex align-items-center text-warning font-weight-bold" v-for="(item,index) in quotationNav" :key="index"
						...
						<navigator class="market-item" v-for="(item, index) in quotation" :key="index" open-type="reLaunch" :url="`/pages/transaction/index?from=market&currency_name=${item.currency_name}&legal_name=${item.legal_name}&currency_id=${item.currency_id}`">
							<view class="left">
								<text class="d-block">
									<text class="font-size-32 font-weight-bold">{{item.currency_name}}</text>
									<text class="font-size-22 opacity-50">/{{item.legal_name}}</text>
