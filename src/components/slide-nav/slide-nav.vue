<template>
	<view class="nav font-size-32 font-weight-bold text-secondary position-relative py-20 text-center" :class="`d-grid-columns-${listLength}`">
		<view class="d-flex justify-content-center align-items-center" :class="{'text-deepblue':activeNav == index}" v-for="(item,index) in list" :key="index" @click="clickNav(index)">
